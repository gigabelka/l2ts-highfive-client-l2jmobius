...
class L2WsClient extends EventTarget {
  constructor(url = null) {
    ...
    // Connection mode: 'shared' (port 3000/ws) or 'standalone' (port 3000)
  ...
  buildWsUrl() {
    ...
    if (this.mode === "standalone") {
      return `${protocol}//${host}:3000${tokenParam}`;
    }
  ...
  connect() {
    ...
    console.log(
      `[WS] Connecting to ${this.url} (attempt ${this.connectionAttempts})`,
    );
  ...
  handleMessage(message) {
    ...
    if (message.type) {
      this.dispatchEvent(
        new CustomEvent(message.type, {
          detail: message.payload || message.data,
        }),
      );
    }
    ...
    if (message.channel) {
      this.dispatchEvent(
        new CustomEvent(`channel:${message.channel}`, {
          detail: message,
        }),
      );
    }
  ...
  subscribe(channels) {
    ...
    this.subscribedChannels = [
      ...new Set([...this.subscribedChannels, ...channels]),
    ];
